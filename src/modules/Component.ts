...
export class Component {
  ...
  setProps = (nextProps: Props) => {
    ...
    const oldPropsJSON = JSON.stringify(this.props);
    // сохраняем старые пропсы
    const oldProps = JSON.parse(oldPropsJSON);
    Object.assign(this.props, nextProps);
    const iseq = isEqual(this.props, oldProps);
    if (!iseq) {
    ...
    Object.values(this.children).forEach((child) => {
      ...
      if (Array.isArray(child)) {
        ...
        for (let a = 0; a < child.length; a++) {
          const zaglushka = fragment.content.querySelector(`[data-id="${child[a]._id}"]`);
